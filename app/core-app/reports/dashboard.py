...
with dash_2:
    ...
    # Compliance Rate
    with columns[0]:
        st.metric(label="Compliance Rate!", value=f"{compliance_rate}%", delta="10%")

    # Annual Clients
    with columns[1]:
        st.metric(label="Annual Clients", value=f"{total_clients:,}", delta="100")

    # Repeat Offender Rate
    with columns[2]:
        st.metric(label="Repeat Offender Rate", value=f"{repeat_offender_rate}%", delta="-9%")
        # style_metric_cards(background_color="white", border_left_color="#DBF227")

    # Inspection Failure Rate
    with columns[3]:
        st.metric(label="Inspection Failure Rate", value=f"{inspection_failure_rate}%", delta="-6%")

    style_metric_cards(background_color="#f9f9f9", border_color="FFF", border_left_color="#DBF227")
